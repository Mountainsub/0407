...
if __name__ == "__main__":
    ...
    while True:
        ...
        calc /= topix_init*0.01
        #dif = calc - pre_calc 
        topix = dde.request("現在値").decode("sjis")
        #dif2 = float(topix) - float(pre_topix)
        
        instance = up_or_down(calc, topix)
        
        """
        if calc  < float(topix)-10 or calc  > float(topix)+10:
            print(calc,"取得結果がズレすぎています。")
            continue
        """
        string = instance.judge()
        print("取得時刻:"+str(now),"計算値:" + str(calc), string)
        store_x.append("consequence",pd.DataFrame({"time":[now], "calc":[calc], "topix":[topix], "string":string}))
        #pre_calc = calc
        #pre_topix = topix
        ...
        if string =="差が大きすぎる":
            topix_init = float(topix_init)* float(calc) *100 / float(topix)
            #1882.4188190928678